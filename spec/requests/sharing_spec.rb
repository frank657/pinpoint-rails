...
RSpec.describe "Sharing & forking", type: :request do
  ...
  let!(:video) { ActsAsTenant.with_tenant(owner_ws) { create(:video, workspace: owner_ws, title: "Back Attacks") } }
  ...
  it "lets the owner create a share link" do
    ...
    expect {
      post app_shares_path, params: { shareable_type: "Video", shareable_id: video.id }
    }.to change(Share, :count).by(1)
    expect(Share.last.token).to be_present
  end

  it "lets a recipient in another workspace view and fork the share" do
    token = ActsAsTenant.with_tenant(owner_ws) { Share.create!(shareable: video, shared_by: owner).token }
    ...
    }.to change { ActsAsTenant.with_tenant(recipient_ws) { Video.count } }.by(1)

    forked = ActsAsTenant.with_tenant(recipient_ws) { Video.find_by(title: "Back Attacks") }
    expect(forked).to be_present
    expect(forked.workspace).to eq(recipient_ws)
    expect(response).to redirect_to(app_video_path(forked))
  end
end
